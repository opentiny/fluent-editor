...
test.describe('Formula.vue', () => {
  ...
  test('should insert formula when formula button is clicked', async ({ page }) => {
    const editor = await page.$('#editor-formula .ql-editor')
    await editor.click()
    ...
    expect(editorHtml).toContain('<span class="ql-formula" data-value="E=mc^2">E=mc^2</span>')
  })
